# Run script 2-prepData.R first

library(rjags)

# Set up data for JAGS
datJ <- list(
  N = 11000000, # Wuhan population
  T = nrow(dat), # Timepoints in the data
  dI = dat$exp, # New exposures (infections)
  dR = dat$rep # New removals (not infectious anymore)
)

model <- jags.model(file = "sens3-model.jag", data = datJ, n.chains = 4, n.adapt = 5000)

res <- coda.samples(model, var=c("beta", "gamma", "asc", "R0", "I", "R"), n.iter=20000)

summ <- summary(res[,c(
    grep("beta", varnames(res)), 
    grep("gamma", varnames(res)), 
    grep("R0", varnames(res)), 
    grep("asc", varnames(res)))])[[2]][,c(3,1,5)]

I <- summary(res[,grep("I\\[", varnames(res))])[[2]][,c(3,1,5)]
R <- summary(res[,grep("R\\[", varnames(res))])[[2]][,c(3,1,5)]

save(res, summ, I, R, file="sens3-results.RData")

